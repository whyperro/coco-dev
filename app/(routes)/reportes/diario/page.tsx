      ...
      <ProtectedRoute roles={['ADMIN', 'AUDITOR']}>
        ...
        <div className='flex flex-col gap-6'>
          <div className='flex justify-between items-center'>
            ...
            {
              tickets && <PDFDownloadLink fileName={`reporte_diario_${format(tickets.date, "yyyy-MM-dd")}`} document={<PdfFile transactionTypeTotals={tickets.transactionTypeTotals} providersReport={tickets.providersReport} paidTickets={tickets?.paidTickets} pendingTickets={tickets?.pendingTickets} clientsReport={tickets.clientsReport} branchReport={tickets.branchReport} date={format(new Date(), "PPP", { locale: es })} />}>
          ...
          <div>
            {
              tickets && <PDFViewer style={{ width: '100%', height: '600px' }}>
                <PdfFile transactionTypeTotals={tickets.transactionTypeTotals} providersReport={tickets.providersReport} paidTickets={tickets?.paidTickets} pendingTickets={tickets?.pendingTickets} branchReport={tickets.branchReport} clientsReport={tickets.clientsReport} date={format(tickets.date, "PPP", { locale: es })} />
              </PDFViewer>
            }
            {
